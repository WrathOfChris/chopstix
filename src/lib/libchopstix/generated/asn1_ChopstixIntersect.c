...
int
encode_ChopstixIntersect(unsigned char *p, size_t len, const ChopstixIntersect *data, size_t *size)
{
size_t ret = 0;
size_t l;
int i, e;
...
int oldret = ret;
ret = 0;
e = encode_general_string(p, len, &(data)->cross, &l);
BACK;
...
int
decode_ChopstixIntersect(const unsigned char *p, size_t len, ChopstixIntersect *data, size_t *size)
{
...
e = decode_general_string(p, len, &(data)->cross, &l);
FORW;
if(dce_fix){

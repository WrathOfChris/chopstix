...
int
encode_ChopstixItemExtra(unsigned char *p, size_t len, const ChopstixItemExtra *data, size_t *size)
{
size_t ret = 0;
size_t l;
int i, e;
...
int
decode_ChopstixItemExtra(const unsigned char *p, size_t len, ChopstixItemExtra *data, size_t *size)
{
...
return 0;
fail:
free_ChopstixItemExtra(data);
return e;
}

void
free_ChopstixItemExtra(ChopstixItemExtra *data)
{
free_ChopstixItemCode(&(data)->code);
}

size_t
length_ChopstixItemExtra(const ChopstixItemExtra *data)
{
size_t ret = 0;
{
int oldret = ret;
...
int
copy_ChopstixItemExtra(const ChopstixItemExtra *from, ChopstixItemExtra *to)
{
*(&(to)->qty) = *(&(from)->qty);
